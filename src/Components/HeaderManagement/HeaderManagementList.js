...
import { faEdit, faChartLine, faTrash, faCog, faPlus, faLanguage, faEye } from '@fortawesome/free-solid-svg-icons';
...
class HeaderManagementList extends React.Component {
	...
	render() {
		...
		const columns = ['sr_no', 'header_desc', 'header_parent_desc','applicable_countries', 'header_from_date', 'header_upto_date', 'header_active_yn', 'header_image_path', 'actions'];
		...
		return (
			<>
				...
				<ServerTable columns={columns} url={url} options={options} addme={$button} bordered hover updateUrl hist_table="SITE_M_HEADER_HIST">
					{
									...
									return (
										<div className="form-control-sm" style={{ textAlign: 'center' }}>
											<DropdownButton id="dropdown-basic-button" title={<FontAwesomeIcon icon={faCog} />}>
												...
												<Dropdown.Item onClick={() => propsObj.editHeader(row.header_id, 'view')}><FontAwesomeIcon icon={faEye} /> View</Dropdown.Item>
