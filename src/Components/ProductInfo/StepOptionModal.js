    ...
    editStepModalRecord = (id, lang = 'en') => {
        ...
        ApiDataService.get(`${api_url}/productStep/${id}/edit`, lang).then(response => {
            ...
            Object.entries(resp).forEach(([key, value]) => {
                if (key == 'step_id') {
                    this.setState({ 'step_id': localStorage.getItem('STEP_ID') });
                } else {
        ...
        return (
            <div>
                <Modal animation={false} size="xl" show={this.props.show} onHide={this.props.closeModal} >
                    ...
                    <Modal.Body>
                        <Form noValidate onSubmit={this.handleSubmit} autoComplete="off" dir={lang_code == 'ar' ? 'rtl' : 'ltr'}>
                            <Row>
                                <Col sm={8}>
                            ...
                            <Col xs={12}>
                                ...
                                <WindowPanel rawHtml={
                                    <div className="windowContent">
                                        <ServerTable renderView={this.state.optionListRenderTable} columns={columns} url={url} options={options} addme={$button} bordered hover updateUrl hist_table="SITE_M_STEP_OPTION_HIST">
                                            {
