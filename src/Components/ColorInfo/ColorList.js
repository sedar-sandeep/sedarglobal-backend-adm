...
class ColorList extends Component {
    ...
    render() {
        ...
        let $button = (<OverlayTrigger overlay={<Tooltip id="tooltip">Add Color</Tooltip>}>
            <button className="btn btn-primary btn-sm" onClick={this.setModalShow}>{<FontAwesomeIcon icon={faPlus} />}</button></OverlayTrigger>);
        const columns = ['sr_no', 'CL_CODE', 'CL_DESC', 'CL_BASE_CODE', 'CL_IMAGE_PATH', 'CL_ACTIVE_YN', 'actions']; //, 'tag_id'
        const options = {
            perPage: PER_PAGE,
            headings: {
                sr_no: '#',
            ...
            sortable: ['CL_CODE', 'CL_DESC', 'CL_ACTIVE_YN'], //'tag_id', 
            ...
            <div>
                ...
                <WindowPanel rawHtml={
                    <div className="windowContent">
                        <ServerTable renderView={this.state.renderTable} columns={columns} url={url} options={options} addme={$button} bordered hover updateUrl hist_table="SITE_M_SHOWROOM_HIST">
                            {
                                function (row, column, index) {
                                    switch (column) {
                                        ...
                                        case 'CL_IMAGE_PATH':
                                            if (row.CL_IMAGE_PATH && row.CL_IMAGE_PATH.match("https://s3-ap-southeast-1.amazonaws.com")) {
                                                return (
                                                    <img src={row.CL_IMAGE_PATH} width="42px" height={'42px'} style={{borderRadius:'50%'}}></img>
                                                );
                                            } else {
                                                return (
                                                    <img src={row.HTTP_IMAGE_PATH + row.CL_IMAGE_PATH} width="42px" height={'42px'} style={{borderRadius:'50%'}}></img>
                                                );
                                            }
                                        case 'actions':
                                            return (
                                                <div className="form-control-sm" style={{ textAlign: 'center' }}>
                                                    <DropdownButton size="sm" id="dropdown-basic-button" title={<FontAwesomeIcon icon={faCog} />}>
                                                        <Dropdown.Item onClick={() => self.editRecord(row.CL_CODE, row.CL_DESC, 'en')}><FontAwesomeIcon icon={faEdit} /> Edit</Dropdown.Item>
                                                        ...
                                                        <Dropdown.Item onClick={() => self.deletRecord(row.CL_CODE)}><FontAwesomeIcon icon={faTrash} /> Delete</Dropdown.Item>
                                                    </DropdownButton>
                                                </div>
                                            );
                                        default:
