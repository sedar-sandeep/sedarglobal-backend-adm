...
class BrandList extends Component {
    ...
    errorThrough = (error, argu) => {
        ...
        var backBrand = ((argu === 'ERR' || argu === 'ERR-OBJ') ? '#ff4c4ceb' : '#20bb20eb');
        this.setState({ snapopen: true, snapcolor: backBrand });
    ...
    render() {
       // let security = this.state.security;
        ...
        let $button = (<OverlayTrigger overlay={<Tooltip id="tooltip">Add Brand</Tooltip>}>
            <button className="btn btn-primary btn-sm" onClick={this.setModalShow}>{<FontAwesomeIcon icon={faPlus} />}</button></OverlayTrigger>);
        const columns = ['sr_no', 'BR_CODE', 'BR_DESC', 'BR_IMAGE_PATH', 'BR_ACTIVE_YN', 'actions']; //, 'tag_id'
        const options = {
            perPage: PER_PAGE,
            headings: {
                sr_no: '#',
            ...
            sortable: ['BR_CODE', 'BR_DESC', 'BR_ACTIVE_YN'], //'tag_id', 
                        ...
                        <ServerTable renderView={this.state.renderTable} columns={columns} url={url} options={options} addme={$button} bordered hover updateUrl hist_table="SITE_M_SHOWROOM_HIST">
                            {
                                function (row, column, index) {
                                    switch (column) {
                                        ...
                                        case 'BR_IMAGE_PATH':
                                            return (
                                                <img src={row.HTTP_IMAGE_PATH + row.BR_IMAGE_PATH} width="50px"></img>
                                            );
                                        case 'actions':
                                            return (
                                                <div className="form-control-sm" style={{ textAlign: 'center' }}>
                                                    <DropdownButton size="sm" id="dropdown-basic-button" title={<FontAwesomeIcon icon={faCog} />}>
                                                        <Dropdown.Item onClick={() => self.editRecord(row.BR_CODE, row.BR_DESC, 'en')}><FontAwesomeIcon icon={faEdit} /> Edit</Dropdown.Item>
                                                        ...
                                                        <Dropdown.Item onClick={() => self.deletRecord(row.BR_CODE)}><FontAwesomeIcon icon={faTrash} /> Delete</Dropdown.Item>
                                                    </DropdownButton>
                                                </div>
                                            );
                                        default:
