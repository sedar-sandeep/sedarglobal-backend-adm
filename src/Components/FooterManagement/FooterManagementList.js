...
class FooterManagementList extends React.Component {
	...
	render() {	
		...
		const columns = ['sr_no', 'footer_desc', 'footer_parent_desc', 'applicable_countries','footer_from_date', 'footer_upto_date', 'footer_active_yn', 'footer_image_path', 'actions'];
		...
		return (
			<>
				...
				<ServerTable columns={columns} url={url} options={options} addme={$button} bordered hover updateUrl hist_table="SITE_M_FOOTER_HIST">
				{				
								...
								return (								
									<div className="form-control-sm" style={{textAlign: 'center'}}>
										<DropdownButton id="dropdown-basic-button" title={<FontAwesomeIcon icon={faCog} />}>
											...
											<Dropdown.Item onClick={() => propsObj.editFooter(row.footer_id, 'view')}><FontAwesomeIcon icon={faEye} /> View</Dropdown.Item>
