...
class BracketModal extends Component {
	...
	editModalRecord = (id) => {
		...
		ApiDataService.get(`${url}/${id}/edit`).then(response => {
			...
			let product_select = this.filterFun(this.state.product_list, 'SPI_PR_ITEM_CODE', resp.SPB_PR_ITEM_CODE ? resp.SPB_PR_ITEM_CODE : 0);
			let bracket_select = this.filterFun(this.state.valance_code_list, 'ITEM_CODE', resp.SPB_CODE ? resp.SPB_CODE : 0);

			this.setState({
				prod_select_product: product_select,
				prod_select_bracket: bracket_select,
			});
		}).catch((error) => {
	...
	SelectFun = (type, val) => {
		if (type === 'SPB_PR_ITEM_CODE' && val.SPI_PR_ITEM_CODE) {
			this.setState({
				prod_select_product: [val],
				SPB_PR_ITEM_CODE: val.SPI_PR_ITEM_CODE
			});
		} else if (type === 'SPB_CODE' && val.ITEM_CODE) {
			this.setState({
				prod_select_bracket: [val],
				SPB_CODE: val.ITEM_CODE
			});
		} else {
			alert('Error!');
		}
	...
	render() {
		...
		let { prod_select_product, product_list, valance_code_list, prod_select_bracket } = this.state;
		return (
			<div>
							...
							<Form.Row>
								<Col xs={6}>
									<Form.Group>
										...
										<Select
											...
											onChange={(e) => this.SelectFun('SPB_PR_ITEM_CODE', e)}
										/>
								...
								<Col xs={6}>
									<Form.Group>
										...
										<Select
											className="basic-single"
											defaultValue={prod_select_bracket.length > 0 && prod_select_bracket ? prod_select_bracket : []}
											value={prod_select_bracket.length > 0 && prod_select_bracket ? prod_select_bracket : []}
											name="SPB_CODE"
											options={valance_code_list}
											getOptionLabel={(val) => val['ITEM_ID'] + ' '+val['ITEM_DESC']}
											getOptionValue={(val) => val['ITEM_CODE']}
											onChange={(e) => this.SelectFun('SPB_CODE', e)}
										/>
