...
import { faEdit, faChartLine, faTrash, faCog, faPlus, faLanguage, faEye,faImages } from '@fortawesome/free-solid-svg-icons';
	...
	render() {	
		...
		return (
			<>
				...
				<ServerTable columns={columns} url={url} options={options} addme={$button}  bordered hover updateUrl hist_table="SITE_M_CATEGORY_HIST">
				{
								...
								return (								
									<div className="form-control-sm" style={{textAlign: 'center'}}>
										<DropdownButton id="dropdown-basic-button" title={<FontAwesomeIcon icon={faCog} />}>
											<Dropdown.Item disabled={self.state.security.UPDATE_YN !== 'Y' ? true : false} onClick={() => propsObj.imagesListFun(row, 'en')}><FontAwesomeIcon icon={faImages} /> Images</Dropdown.Item>
											<Dropdown.Item disabled={self.state.security.UPDATE_YN !== 'Y' ? true : false} onClick={() => propsObj.editCategory(row.cate_id, 'en')}><FontAwesomeIcon icon={faEdit} /> Edit</Dropdown.Item>
											<Dropdown.Item onClick={() => propsObj.editCategory(row.cate_id, 'en', 'view')}><FontAwesomeIcon icon={faEye} /> View</Dropdown.Item>
