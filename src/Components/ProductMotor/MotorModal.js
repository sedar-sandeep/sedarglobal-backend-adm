...
class MotorModal extends Component {
	...
	editModalRecord = (id) => {
		...
		ApiDataService.get(`${url}/${id}/edit`).then(response => {
			...
			let product_select = this.filterFun(this.state.product_list, 'SPI_PR_ITEM_CODE', resp.SPM_PR_ITEM_CODE ? resp.SPM_PR_ITEM_CODE : 0);

			let motor_select = this.filterFun(this.state.motor_code_list, 'ITEM_CODE', resp.SPM_MOTOR_CODE ? resp.SPM_MOTOR_CODE : 0);
			let concrete_select = this.filterFun(this.state.bracket_concrete, 'SPB_CODE', resp.SPM_EPB_CODE_CON ? resp.SPM_EPB_CODE_CON : 0);
			let gypsum_select = this.filterFun(this.state.bracket_gypsum, 'SPB_CODE', resp.SPM_EPB_CODE_GYP ? resp.SPM_EPB_CODE_GYP : 0);
			let wood_select = this.filterFun(this.state.bracket_wood, 'SPB_CODE', resp.SPM_EPB_CODE_ALU ? resp.SPM_EPB_CODE_ALU : 0);

			this.setState({
				prod_select_product: product_select,
				prod_select_motor: motor_select,
				prod_select_concrete: concrete_select,
				prod_select_gypsum: gypsum_select,
				prod_select_wood: wood_select,
			});
		}).catch((error) => {
	...
	SelectFun = (type, val) => {
		if (type === 'SPM_PR_ITEM_CODE' && val.SPI_PR_ITEM_CODE) {
			this.setState({
				prod_select_product: [val],
				SPM_PR_ITEM_CODE: val.SPI_PR_ITEM_CODE
			});
		} else if (type === 'SPM_MOTOR_CODE' && val.ITEM_CODE) {
			this.setState({
				prod_select_motor: [val],
				SPM_MOTOR_CODE: val.ITEM_CODE
			});
		} else if (type === 'SPM_EPB_CODE_CON' && val.SPB_CODE) {
			this.setState({
				prod_select_concrete: [val],
				SPM_EPB_CODE_CON: val.SPB_CODE
			});
		} else if (type === 'SPM_EPB_CODE_GYP' && val.SPB_CODE) {
			this.setState({
				prod_select_gypsum: [val],
				SPM_EPB_CODE_GYP: val.SPB_CODE
			});
		} else if (type === 'SPM_EPB_CODE_ALU' && val.SPB_CODE) {
			this.setState({
				prod_select_wood: [val],
				SPM_EPB_CODE_ALU: val.SPB_CODE
			});
		}
		else {
			alert('Error!');
	...
	render() {
		...
		let { prod_select_product, product_list, motor_code_list, prod_select_motor, bracket_concrete, bracket_gypsum, bracket_wood, prod_select_concrete, prod_select_gypsum, prod_select_wood } = this.state;
		return (
			<div>
							...
							<Form.Row>
								<Col xs={7}>
									<Form.Group>
										...
										<Select
											...
											name="SPM_PR_ITEM_CODE"
											options={product_list}
											getOptionLabel={(product_list) => product_list['SPI_DESC'] + ' ' + product_list['ITEM_ID']}
											getOptionValue={(product_list) => product_list['SPI_PR_ITEM_CODE']}
											onChange={(e) => this.SelectFun('SPM_PR_ITEM_CODE', e)}
										/>
								...
								<Col xs={6}>
									<Form.Group >
										...
										<Select
											className="basic-single"
											defaultValue={prod_select_motor.length > 0 && prod_select_motor ? prod_select_motor : []}
											value={prod_select_motor.length > 0 && prod_select_motor ? prod_select_motor : []}
											name="SPM_MOTOR_CODE"
											options={motor_code_list}
											getOptionLabel={(val) => val['ITEM_ID'] + ' ' + val['ITEM_DESC']}
											getOptionValue={(val) => val['ITEM_CODE']}
											onChange={(e) => this.SelectFun('SPM_MOTOR_CODE', e)}
										/>
								...
								<Col xs={4}>
									<Form.Group >
										...
										<Form.Control as="select" value={setValue.SPM_COMPONENT_TYPE} name="SPM_COMPONENT_TYPE" onChange={this.stateChanges}>
											<option value="">Select Component Type</option>
											{component_type_list && component_type_list.map((data, i) => (
												<option value={data.value} key={i}>{data.lable}</option>
											))}
										</Form.Control>
										{this.state.errors["SPM_COMPONENT_TYPE"] &&
											<span className='custError'>{this.state.errors["SPM_COMPONENT_TYPE"]}</span>
										}
									</Form.Group>
								</Col>
								...
								<Col xs={8}>
									<Form.Group >
										...
										<Select
											className="basic-single"
											defaultValue={prod_select_concrete.length > 0 && prod_select_concrete ? prod_select_concrete : []}
											value={prod_select_concrete.length > 0 && prod_select_concrete ? prod_select_concrete : []}
											name="SPM_EPB_CODE_CON"
											options={bracket_concrete}
											getOptionLabel={(val) => val['SPB_DESC']}
											getOptionValue={(val) => val['SPB_CODE']}
											onChange={(e) => this.SelectFun('SPM_EPB_CODE_CON', e)}
										/>
								...
								<Col xs={6}>
									<Form.Group>
										...
										<Select
											className="basic-single"
											defaultValue={prod_select_gypsum.length > 0 && prod_select_gypsum ? prod_select_gypsum : []}
											value={prod_select_gypsum.length > 0 && prod_select_gypsum ? prod_select_gypsum : []}
											name="SPM_EPB_CODE_GYP"
											options={bracket_gypsum}
											getOptionLabel={(val) => val['SPB_DESC']}
											getOptionValue={(val) => val['SPB_CODE']}
											onChange={(e) => this.SelectFun('SPM_EPB_CODE_GYP', e)}
										/>
								...
								<Col xs={6}>
									<Form.Group>
										...
										<Select
											className="basic-single"
											defaultValue={prod_select_wood.length > 0 && prod_select_wood ? prod_select_wood : []}
											value={prod_select_wood.length > 0 && prod_select_wood ? prod_select_wood : []}
											name="SPM_EPB_CODE_ALU"
											options={bracket_wood}
											getOptionLabel={(val) => val['SPB_DESC']}
											getOptionValue={(val) => val['SPB_CODE']}
											onChange={(e) => this.SelectFun('SPM_EPB_CODE_ALU', e)}
										/>
