...
class MotorBuildModal extends Component {
    ...
    editRecord = (id) => {
        ...
        ApiDataService.get(`${url}/${id}/edit`).then(response => {
            ...
            let motor_select = this.filterFun(this.state.motor_code_list, 'ITEM_CODE', resp.SPMC_MOTOR_CODE ? resp.SPMC_MOTOR_CODE : 0);
            this.setState({
    ...
    SelectFun = (type, val) => {
        if (type === 'SPMC_MOTOR_CODE' && val.ITEM_CODE) {
            this.setState({
                prod_select_motor: [val],
                SPMC_MOTOR_CODE: val.ITEM_CODE
            });
        } else {
            alert('Error!');
        }
    ...
    render() {
        ...
        let { motor_code_list, prod_select_motor } = this.state;
