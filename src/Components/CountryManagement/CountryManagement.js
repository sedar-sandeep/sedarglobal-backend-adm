...
class CountryManagement extends Component {
	...
	openModal = () => {
		this.setState({ isOpen: true, lang_code: 'en' });
	}
	closeModal = () => {
		if (this._isMounted){
			this.setState({ 
	...
	editCountry = (countryISO, lang_code) => {	
		this.setState({ lang_code: lang_code });
		this.getLanguagesData();
		let $url = `${apiUrl}/${countryISO}/edit?language=${lang_code}`;
		ApiDataService.get($url)
		.then(res => {		
			if(res.data.return_status==="0"){
				if (this._isMounted){
	...
	geteditCountry = (countryISO, lang_code) => {
		this.setState({ lang_code: lang_code });
		let $url = `${apiUrl}/${countryISO}/edit?language=${lang_code}`;
		ApiDataService.get($url)
			.then(res => {
	...
	render() {	
		...
		if(this.state.isAddCountry || this.state.isEditCountry) {
		  countryForm = <AddUpdateCountryManagement 
			...
			geteditCountry={this.geteditCountry}
		/>
		}
		let seoForm;	
