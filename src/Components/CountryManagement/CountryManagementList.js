	...
	render() {	
		...
		return (
			<>
				...
				<ServerTable columns={columns} url={url} options={options} addme={$button} bordered hover updateUrl hist_table="SITE_M_COUNTRY_HIST">
				{
								...
								return (								
									<div className="form-control-sm" style={{textAlign: 'center'}}>																
										<DropdownButton id="dropdown-basic-button" title={<FontAwesomeIcon icon={faCog} />}>
											<Dropdown.Item disabled={self.state.security.UPDATE_YN !== 'Y' ? true : false} onClick={() => propsObj.editCountry(row.country_iso, 'en')}><FontAwesomeIcon icon={faEdit} /> Edit</Dropdown.Item>
											...
											<Dropdown.Item disabled={self.state.security.LANGUAGE_YN !== 'Y' ? true : false} onClick={() => propsObj.editCountry(row.country_iso, 'ar')}><FontAwesomeIcon icon={faLanguage} /> Edit Language</Dropdown.Item>
