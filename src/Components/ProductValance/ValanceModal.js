...
class ValanceModal extends Component {
	...
	editModalRecord = (id) => {
		...
		ApiDataService.get(`${url}/${id}/edit`).then(response => {
			...
			let product_select = this.filterFun(this.state.product_list, 'SPI_PR_ITEM_CODE', resp.SPV_PR_ITEM_CODE ? resp.SPV_PR_ITEM_CODE : 0);
			this.setState({ prod_select_product: product_select });
	...
	SelectFun = (type, val) => {
		if (type === 'SPV_PR_ITEM_CODE' && val.SPI_PR_ITEM_CODE) {
			this.setState({
				prod_select_product: [val],
				SPV_PR_ITEM_CODE: val.SPI_PR_ITEM_CODE
			});
		} else {
			alert('Error!');
		}
	...
	render() {
		...
		let { prod_select_product, product_list } = this.state;
								...
								<Col xs={7}>
									<Form.Group>
										...
										<Select
											...
											onChange={(e) => this.SelectFun('SPV_PR_ITEM_CODE', e)}
										/>
