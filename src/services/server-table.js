...
import { faFilter, faPlusSquare, faMinusSquare, faHistory } from '@fortawesome/free-solid-svg-icons';
...
class ServerTable extends Component {
            ...
            if (childrowDefine[column] !== 'hideColumn') {
                return (<th key={column}
                    ...
                    {options.sortable.includes(column) &&
                        <div className="sortOrder" key={inx}>
            ...
            columns.map((column, index) => {
                ...
                if (childrowDefine[column] !== 'hideColumn') {
                    if (index === 0 && showChildButton) {
                        spantag = (<span key={index} onClick={(e) => self.expandChild(e, row_index)} className="childExpand"><FontAwesomeIcon icon={self.state.childshowhide === row_index ? faMinusSquare : faPlusSquare} /></span>);
                    }
    ...
    SelectFun = (type, val) => {
        ...
        if (type === 'product_list') {
            requestData.search_column = 'sii_item_id';
            requestData.addon_search_column = val.value;
            requestData.page = 1;
        }
        this.setState({ requestData: requestData, isLoading: true }, () => {
            this.handleFetchData();
        ...
        return (
            <>
                                ...
                                <div className="form-group- float-right">
                                    <div className="mb-0 input-group">
                                        {(this.state.addonList.length > 1) &&
                                            <div className="custdropdwn" style={{ marginRight: '15px', width: '280px' }}>
                                                ...
                                                <Select
                                                    // ref={this.table_addon_search_column}
                                                    ...
                                                    name="product_list"
                                                    options={this.state.addonList}
                                                    getOptionLabel={(val) => val['label']}
                                                    getOptionValue={(val) => val['value']}
                                                    onChange={(e) => this.SelectFun('product_list', e)}
                                                />
                                            </div>
                                        ...
                                        <div className="input-group-append history-btn">
                                            <button id="btn-id" onClick={() => this.handleHistory()} className="ml-2 btn btn-danger form-control form-control-sm" style={{ fontSize: '0.9rem' }}><FontAwesomeIcon icon={faHistory} /></button >
                                        </div>
                                    </div>
                                </div>
                            }
                ...
                <History
                    ref={this.modalRef}
                    url={this.props.url}
                    hist_table={this.props.hist_table}
